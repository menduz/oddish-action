    ...
    function patch(fs2) {
      ...
      function statFix(orig) {
        ...
        return function(target, cb) {
          return orig.call(fs2, target, function(er, stats) {
            if (!stats)
      ...
      function statFixSync(orig) {
        ...
        return function(target) {
          var stats = orig.call(fs2, target);
          if (stats.uid < 0)
            stats.uid += 4294967296;
          if (stats.gid < 0)
            stats.gid += 4294967296;
...
var require_graceful_fs = __commonJS({
  "node_modules/graceful-fs/graceful-fs.js"(exports2, module2) {
    ...
    module2.exports.close = function(fs$close) {
    ...
    module2.exports.closeSync = function(fs$closeSync) {
      return function(fd) {
    ...
    if (!/\bgraceful-fs\b/.test(fs2.closeSync.toString())) {
      fs2.closeSync = module2.exports.closeSync;
      fs2.close = module2.exports.close;
    }
    function patch(fs3) {
      polyfills(fs3);
...
// node_modules/git-rev-sync/index.js
var require_git_rev_sync = __commonJS({
  "node_modules/git-rev-sync/index.js"(exports2, module2) {
    "use strict";
    ...
    function _getGitDirectory(start) {
      ...
      if (fs2.existsSync(testPath)) {
        if (!fs2.statSync(testPath).isDirectory()) {
          var parentRepoPath = fs2.readFileSync(testPath, "utf8").trim().split(" ").pop();
          if (!path.isAbsolute(parentRepoPath)) {
            parentRepoPath = path.resolve(gitRepoPath, parentRepoPath);
          }
          if (fs2.existsSync(parentRepoPath)) {
    ...
    function branch(dir) {
      ...
      var head = fs2.readFileSync(path.resolve(gitDir, "HEAD"), "utf8");
    ...
    function long(dir) {
      ...
      var ref;
      if (fs2.existsSync(refsFilePath)) {
        ref = fs2.readFileSync(refsFilePath, "utf8");
      } else {
        var refToFind = ["refs", "heads", b].join("/");
        var packfileContents = fs2.readFileSync(path.resolve(gitDir, "packed-refs"), "utf8");
...
var git = require_git_rev_sync();
var fs = require("fs");
var os = require("os");
var commitHash = (0, import_child_process2.execSync)("git rev-parse HEAD").toString().trim();
async function setCommitHash() {
  const packageJson = JSON.parse(fs.readFileSync("package.json").toString());
  packageJson.commit = commitHash;
  fs.writeFileSync("package.json", JSON.stringify(packageJson, null, 2));
}
var time = new Date().toISOString().replace(/(\..*$)/g, "").replace(/([^\dT])/g, "").replace("T", "");
console.log(`> oddish`);
function configAuthentication(registryUrl, alwaysAuth, workingDirectory) {
  const npmrc = (0, import_path.resolve)(process.env["RUNNER_TEMP"] || workingDirectory, ".npmrc");
...
async function execute(command, workingDirectory) {
  return core.group(`${command}`, () => new Promise((onSuccess, onError) => {
    (0, import_child_process.exec)(command, {cwd: workingDirectory}, (error, stdout, stderr) => {
...
async function getSnapshotVersion(workingDirectory, registryUrl) {
...
async function getReleaseTags(workingDirectory, registry) {
  try {
    const json = JSON.parse(fs.readFileSync(workingDirectory + "/package.json", "utf8"));
    const versions = await (0, import_node_fetch.default)(`${registry}/-/package/${json.name}/dist-tags`);
    if (versions.ok) {
...
var run = async () => {
  ...
  let workingDirectory = (0, import_path.resolve)(core.getInput("cwd") || process.cwd());
  ...
  let branch = process.env.CIRCLE_BRANCH || process.env.BRANCH_NAME || process.env.TRAVIS_BRANCH || await getBranch(workingDirectory);
  let npmTag = null;
  let gitTag = null;
